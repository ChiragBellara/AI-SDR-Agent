...
class ReadinessResearcher(BaseResearcher):
    ...
    async def analyze(self, state: ResearchState):
        ...
        queries = []
        async for event in self.generate_queries(state, READINESS_ANALYZER_QUERY_PROMPT):
            yield event
            if event.get("type") == "queries_complete":
                queries = event.get("queries", [])
        
        # Log subqueries
        subqueries_msg = "🔍 Subqueries for readiness analysis:\n" + "\n".join([f"• {query}" for query in queries])
        state.setdefault('messages', []).append(AIMessage(content=subqueries_msg).model_dump())
        
        # Start with site scrape data
        readiness_data = dict[str, Any](state.get('site_scrape', {}))
        ...
        readiness_data.update(documents)
        
        # Update state
        completion_msg = f"Readiness Scanner found {len(readiness_data)} documents for {company}"
        state.setdefault('messages', []).append(AIMessage(content=completion_msg).model_dump())
        state['readiness_data'] = readiness_data
        
        yield {"type": "analysis_complete", "data_type": "readiness_data", "count": len(readiness_data)}
        yield {'message': [completion_msg], 'readiness_data': readiness_data}
